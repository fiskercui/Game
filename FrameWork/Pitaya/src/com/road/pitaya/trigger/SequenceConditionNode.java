 ...
 * @param <M>
 * @param <N>
 *            顺序结点,根据逻辑关系字段决定怎么样进行判定
 */
public class SequenceConditionNode extends AiTreeNode<EventArg, Boolean>
{
    ...
    public SequenceConditionNode(ConditionInfo conditionInfo,
            IAiNodeFactory<EventArg, Boolean> nodeFactory)
    {
        super();
        ...
        if (sonConditionInfos != null && sonConditionInfos.size() > 0)
        {
            for (ConditionInfo sonCondition : sonConditionInfos)
            {
                ...
                else
                {
                    ...
                    ConditionNode<EventArg> sonNode = (ConditionNode<EventArg>) nodeFactory.createAiTreeNode(sonCondition.getConditionID(),
                                                                                           isNeedNew,
    ...
    @Override
    public Boolean actionExecut(EventArg context)
    {
     ...
     * @return
     */
    private boolean andNode(EventArg context)
    {
        for (AiTreeNode<EventArg, Boolean> sonNode : sonNodes)
        {
     ...
     * @return
     */
    private boolean orNode(EventArg context)
    {
        for (AiTreeNode<EventArg, Boolean> sonNode : sonNodes)
        {
